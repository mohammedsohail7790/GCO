...
### 2.9 5xx errors leaked raw internal exception messages to the client (CONFIRMED, FIXED)

**File**: `lib/api/response.ts::handleRouteError`

Any unhandled exception (a bug, a Prisma internals error, anything not deliberately thrown via `fail()`) had its raw `.message` sent directly in the JSON response body to the caller, regardless of status code. For a genuine 500 this could leak internal detail (query shape, field names, occasionally more) with no legitimate reason to leave the server. Fixed: 5xx responses now always return a generic "Internal server error" to the client; the full message + stack is still logged server-side via the structured logger, which is where it's actually useful.

### 2.10 Validation errors and malformed JSON were misclassified as 500 (CONFIRMED, FIXED)

Fixing 2.9 surfaced this: every route validates its body with `Schema.parse(await req.json())`, but neither a `ZodError` (invalid input) nor a `SyntaxError` (malformed JSON) carries a `.status` field, so both fell through to the generic `status: 500` branch — meaning basic client input errors (bad email format, empty password, unparseable JSON) were returning `500 Internal Server Error` instead of `400 Bad Request` **on every route in the application**. Reproduced: `POST /auth/login` with `{"email":"not-an-email"}` returned 500 before the fix. Fixed by explicitly classifying `ZodError` and `SyntaxError` as 400 in `handleRouteError`, with the validation detail preserved in the response (safe — it's the caller's own input being echoed back, not internal state). Covered by `tests/unit/errorClassification.test.ts`.

### 2.8 Realtime server was unusable from any browser (CONFIRMED, FIXED)

`workers/realtime-server.ts` expected the REST API's access token (`gco_at`) as a WebSocket URL query param. But `gco_at` is an `httpOnly` cookie by design (XSS protection) - client-side JavaScript cannot read it, so no browser could ever have actually connected. This was previously undetectable because nothing had tried to wire it into the UI yet. Fixed by adding a distinct, very-short-lived (30s) "realtime ticket" token type (`lib/auth/tokens.ts::signRealtimeTicket`/`verifyRealtimeTicket`, explicitly rejected by the REST API's `verifyAccessToken` so a leaked ticket - e.g. logged in a WS URL - can't be replayed against any REST endpoint) issued by a new authenticated route (`GET /api/v1/realtime/ticket`) that reads the httpOnly cookie server-side and hands the ticket back in the JSON body, which JS *can* read. Verified end-to-end: a real webhook's `message.received`/`assignment.created` events were observed arriving over the WebSocket in a real browser tab within under a second of the webhook firing, well ahead of the 5s polling fallback that remains active regardless.

### 2.7 Hardcoded Postgres password directly in a committed `docker-compose.yml` (CONFIRMED, FIXED)

`POSTGRES_PASSWORD: gco_dev_password` was a literal value in `docker-compose.yml`, which is a tracked, committed file - not a template. Fixed to `${POSTGRES_PASSWORD:?...}`, sourced from `.env` (docker compose reads a `.env` file in the same directory automatically), with the required var also documented in `.env.example`. Found during the production-configuration review in this pass, after `docker-compose.yml` had already been committed to git in the initial commit - worth noting for anyone reviewing that commit's history.
